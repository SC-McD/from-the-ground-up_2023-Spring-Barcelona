...
""" 
Data from [Estimation of spatial distribution of irrigated crop areas in Europe for large-scale modelling applications]
...
rice_Coimbra.new_edge(
    ...
    temporal_distribution=TD(
        amount=np.ones(12)*0.5,
        date=np.linspace(-12, 1, 12, dtype="timedelta64[M]", endpoint=False))
).save()

# make one in a different region
rice_Sevilla = food.new_node(
    code="rice_Sevilla",
    name="rice_Sevilla",
    location=('regions', 'Sevilla')
)
rice_Sevilla.save()

rice_Sevilla.new_edge(
    input=water,
    amount=6,
    type="biosphere",
    temporal_distribution=TD(
...
meal_Sevilla = food.new_node(
    code="meal_Sevilla",
    name="meal_Sevilla",
    location=('cities', '14')
)
meal_Sevilla.save()
# meal_Sevilla.new_edge(
#     input=water,
#     amount=0,
#     type="biosphere",
# ).save()
...
meal_Sevilla = bd.get_node(name="meal_Sevilla")
...
GLOBAL_CF = 0.44  # taken from the paper above
...
rice_Coimbra = bd.get_node(name="rice_Coimbra")
for x in rice_Coimbra.biosphere():
...
# define a function to calculate the characterization matrix
...
# dodgy but works for this example (there are some bw2_temporalis bugs)
tl.df['flow'] = 1.0
tl.df['activity'] = 3
...
plt.ylabel("Water stress (L(eq)/kg deprivation)")
plt.xlabel("Date")
